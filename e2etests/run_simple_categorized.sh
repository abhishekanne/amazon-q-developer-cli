...
run_category() {
    ...
    if [ "$QUIET_MODE" = false ]; then
        ...
        cargo test --tests --features "$category" --features "sanity" -- --test-threads=1 2>&1 | while IFS= read -r line; do
        ...
        cargo test --tests --features "$category"  --features "sanity" -- --nocapture --test-threads=1 2>&1 | while IFS= read -r line; do
            echo "$line"
            if echo "$line" | grep -q "test .* \.\.\. ok$"; then
                test_name=$(echo "$line" | sed 's/test \(.*\) \.\.\. ok/\1/')
